 ...
 *
 * ## ⚠️ 只有一条,不是「先放一条以后再补」—— 这是扫过之后的结果
 *
 * 扫的判据是**无界循环构造**(`for (;;)` / `while (true)` / 由服务端数据驱动的
 * 递归),因为只有那里才存在「一条上限在扛着终止性」这回事。全仓(排除
 * `test/` 与 `dist/`)一共两处:
 *
 * | 位置 | 谁让它终止 | 进不进这张表 |
 * | --- | --- | --- |
 * | `console-web/src/api.ts` 的 `for (;;)` | **`MAX_PAGES`** | ✅ 就是下面这条 |
 * | `sdk/typescript/src/client.ts` 的 `while (true)` | 流读到 `done` / 调用方 abort | ❌ 没有可去掉的上限 |
 *
 * 另外两处看着像上限、其实不是,一并写明免得下一个人以为漏了:
 *
 * - `packages/supervisor` 的 `MAX_PROCESSES_CEILING` —— 它截的是一个**数值**,
 *   去掉之后进程数变大,不会不终止。它的失效形态是「红」,归常规负向验证。
 * - `packages/webhooks` 的 `retries` —— 终止性由 `for` 的计数结构本身承担,
 *   不是由某个常量;去掉那个常量的写法根本不成立。
 */
const CAPS = [
  {
