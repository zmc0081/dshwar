...
| 进程(一 principal 一进程) | 跨信任边界             | 顺带解决上游 **stdio SDK 协议**无 cancel 的问题 |
| 容器(进程 + OS 沙箱)      | 多租户 SaaS            |                                                 |

> ⚠️ **「无 cancel」只适用于走 stdio SDK 协议的消费方。** V0.2.0 Session 0 实测:
> **进程内**的 `Agent` 接口有 `cancel(cause)`,`AgentHandle.dispose()` 亦然,两者都真的截断输出。
> DSHWAR 网关走进程内驱动,不受此限。见 `docs/FEASIBILITY-REPORT-V2.md` §4.1。
> 进程隔离的**第一个**理由(跨信任边界的安全隔离)不受影响,仍在 V0.4.0。
