...
**判据**:凡是按**文本形状**判定的守卫(grep 族、正则族、行扫描族),
落地前必须先确认一件事 ——

> **它会不会拦住「对这个形状的说明」?**

因为一条守卫要拦的形状,恰恰是最值得在注释、决策文档、任务书里**写下来**的那个。
...
**落到操作上**:写一条文本形状守卫时,把判据放到夹具里跑之前,先 grep 一遍
仓库里**讲这个形状**的地方(注释、决策文档、任务书),确认一处都不会被点名。
点到了就说明判据缺一个「这是说明,不是代码」的维度。
