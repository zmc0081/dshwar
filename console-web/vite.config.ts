 ...
 * dev server —— 用来在真实浏览器里把九屏跑起来看。真正的打包(三宿主、
 * Tauri sidecar)是后面单独排的 Session,不在本 Session。
 *
...
export default defineConfig({
  ...
  server: {
    // 4321 是 design-system 实测台占的,4322 是工作台。三个 dev server 会同时开着。
    port: 4323,
     ...
     * ## 为什么需要它
     *
     * 网关不发 CORS 头。dev server 在 :4323、网关在 :8787,直接 fetch 会得到
     * `TypeError: Failed to fetch` —— 实测过,而且那个错误信息**不提 CORS**,
     * 与「网关没起来」长得一模一样。
     *
     * ⚠️ 对运营后台还有第二层:预检。Admin 面带的是自定义头
     * `x-dshwar-admin-key`,它让请求**不再是简单请求** —— 浏览器会先发一个
     * `OPTIONS`。跨源时那个 OPTIONS 没有 CORS 头可回,于是真正的 GET
     * **一次都不会发出去**;开发者在网关日志里什么都看不到,
     * 而浏览器只说一句 fetch 失败。同源之后这一层整个不存在。
     *
