...
提示词注入、恶意 MCP、污染 skill 都可能越界。

**而 V0.4.6 的实测发现问题比「越界」更靠前**:逻辑档下 principal 根本到不了 agent
执行层,多用户的文件全落进 `anonymous/anonymous/` 互相静默覆盖。所以逻辑档
**只支持单个 principal**,V0.4.7 起配多用户身份直接拒绝启动。

这不再是「挡在一家公司内部」——**本包是多用户的唯一可选项**。
