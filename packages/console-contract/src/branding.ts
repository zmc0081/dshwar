 ...
 *
 * 🔗 **本文件与 Claude Design 里名为 “DSHWAR” 的设计系统是同一份契约的两个副本,
 * 之间没有任何自动同步。** 改这里必须同步改那边,反之亦然 ——
 * 约定与后果见 `docs/DECISIONS/design-system-sync.md`。
 *
...
export interface TenantBranding {
  ...
  /**
   * 品牌主色,`#RRGGBB`。**`null` = 未配置。**
   *
   * ★ **只收种子色,色阶由客户端确定性派生** —— 派生算法不在契约里。
   * 放进契约等于允许两端各派生一套,而那两套迟早会不一样。
   *
   * ## 为什么是 `string | null` 而不是一个默认色
   *
   * `null` 表示**未配置**,渲染成**中性外观(无彩)** —— 那不是缺省的
   * 半成品,是一个**受支持的完整形态**(开源自建模式的默认就是它)。
   *
   * 用一个哨兵默认值(此前是 `#2F6FEB`)去表示「没配」的问题与 V0.6.0
   * 的发票卖方字段**同形**:「未配置」与「配置成某个值」必须在**类型层**
   * 可分。靠哨兵值区分是一条**运行时约定**,而运行时约定会被下一个人
   * 当成普通值 —— 他没有理由知道 `#2F6FEB` 有特殊含义。
   *
   * ⚠️ **禁止 `?? '#2F6FEB'` 之类的兜底**:那会把类型层刚区分开的两种
   * 状态在第一个调用点重新合并。判空**收敛在派生入口一处**,不在调用点判。
   *
   * ## 写入时校验:三项下限,不达标拒绝保存
   *
   * 校验的是**派生结果**,不是种子色本身 —— 种子色好不好看不是判据,
   * 派生出来的角色令牌能不能读才是:
   *
   * | 令牌                  | 对比度下限 |
   * | --------------------- | ---------- |
   ...
   * 只是这次「不安全」指的是**读不了**。
   *
   * ⚠️ **实心填充若落在中亮带 `L ∈ (0.55, 0.80)` 被重打光**:保存前必须
   * **回显原色与重打光结果并要求管理员确认**。
   * **这是阻塞步骤,不是提示** —— 一个可以划过去的提示等于没有:
   * 管理员会在「我设的颜色没生效」的工单里才第一次注意到它。
   */
...
/**
 * 配置界面的**建议起点**,给管理员一个能直接用的值。
 *
 * ⚠️ **它刻意不是 {@link NEUTRAL_BRANDING} 的值。**
 * 「建议」与「默认」是两回事:默认值会让「未配置」不可见,
 * 而建议值只在管理员**主动采纳**时才成为配置 —— 采纳这个动作本身
 * 就把状态从「未配置」变成了「配置成这个值」,两者仍然可分。
 *
 * ## 为什么是这个值(实测)
 *
 * | | L | C | H | 派生策略 |
 * | --- | --- | --- | --- | --- |
 * | 旧默认 `#2F6FEB` | **0.573** | 0.199 | 261.8° | 🚨 落在中亮带 → **RE-LIT** |
 * | 本值 `#1D5BD4` | **0.509** | 0.197 | 261.9° | ✅ 带外 → **VERBATIM** |
 *
 * 取自旧默认色所派生 ramp 的 **700 档**,所以色相几乎不动(漂移 0.1°)、
 * 彩度也贴近 —— 观感上是同一个蓝,只是压暗到中亮带之外。
 *
 * 六个角色令牌全部达标(数据来自设计系统侧的派生实现,见 §同步约定)。
 *
 * ⚠️ **旧默认值命中了契约自己禁止的分支** —— 契约一边要求「落在中亮带
 * 要阻塞确认」,一边把一个带内的值设成默认,于是那条确认流程在默认路径上
 * **永远走不到**,等于半条死代码。本值的第一要求就是**带外**。
 *
 * 「将来有人改这个值又改回带内」由 `test/branding.test.ts` 的带外断言守着。
 */
export const SUGGESTED_PRIMARY_COLOR = '#1D5BD4'
...
export const NEUTRAL_BRANDING: TenantBranding = {
  ...
  logoDark: null,
  favicon: null,
  // null = 未配置 → 中性外观(无彩)。**不要在这里放一个默认色** ——
  // 那正是 V0.8.0 回写掉的那个哨兵。见 TenantBranding.primaryColor 的说明。
  primaryColor: null,
  accentColor: null,
  supportUrl: null,
  supportEmail: null,
