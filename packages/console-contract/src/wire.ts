 ...
 * **领域类型能满足线上类型**,不是相等。
 *
 * ## ⚠️ 一处**有意的例外**:`console-web` 直接依赖 `@dshwar/metering`
 *
 * 上面第 1 条说的是「消费方被迫装整个运行时」。V0.9.0 Session 5.5 之后
 * `console-web` 的 `package.json` 里确实出现了 `@dshwar/metering` ——
 * **那不是这条表态被违反了**,是它的适用范围被说清楚了。
 *
 * | | 表态针对的 | 这一次的 |
 * | --- | --- | --- |
 * | 依赖对象 | `@dshwar/subject` 等**服务端运行时**包 | `@dshwar/metering` |
 * | 拖进来的东西 | cordis、storage 后端、进程模型 | **零依赖、无 `node:` import** 的纯 TS |
 * | 前端用它做什么 | 需要整个领域模型 | 只用一个纯函数 `readCost()` |
 *
 * 换成「在这里再实现一遍窄化」的代价更大:**钱的语义会有两份实现**,
 * 而这个字段的整个历史(`costMinorUnits` 的两个洞)正是语义分家的产物 ——
 * 「算不出来」被读成 0、`÷ 100` 各写各的。判据是
 * **「重复的是数据形状,还是判断规则」**:
 * 形状可以投影(那是本文件在做的事),**判断规则不许有第二份**。
 *
 * ⇒ {@link ConsoleCost} 仍然是一个**结构镜像**(本文件不 import 领域类型),
 * 而把线上形状读回领域形状的那条规则只有一份,在 `@dshwar/metering/cost.ts`。
 * `test/conformance.test.ts` 里一条**往返断言**钉着两者。
 *
