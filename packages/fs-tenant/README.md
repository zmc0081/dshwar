...
**逻辑隔离只支持单个 principal**(V0.4.7)。多用户必须用进程隔离,跨信任边界还要加容器。

> ⚠️ 这句话原本是「仅适用于互相信任的用户」。那个判据是错的:信任与否说的是恶意方
> 能不能越界,而逻辑档下多用户是**根本没有分隔** —— 所有人落进 `anonymous/anonymous/`
> 互相静默覆盖,本包的路径钉死在那种情形下拿不到区分用的 principal。
