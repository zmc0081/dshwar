...
> 当前版本(正在开发): **V0.4.5** —— 本行强制为"正在开发的版本号",随新版本规划立即更新(见第三部分强制约束)
...
| Session                             | 状态      | 说明                               |
| ----------------------------------- | --------- | ---------------------------------- |
| 0 可行性证伪:跨进程驱动             | ⬜ 未开始 | **止损点** —— 子进程里还能不能驱动 |
| 1 `@dshwar/supervisor` 契约与进程池 | ⬜ 未开始 | spawn / 守护 / 回收                |
| 2 跨进程会话驱动                    | ⬜ 未开始 | 事件回传、取消、崩溃恢复           |
| 3 隔离级别与网关接线                | ⬜ 未开始 | profile 三档,治理联动              |
| 4 文档与发布准备                    | ⬜ 未开始 | 隔离矩阵改写,这是对外承诺的变更    |
