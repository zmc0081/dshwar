...
## <span style="color:#d00000">●</span> M0.5.5 · 工作台后端(Session 0-4) <span style="color:#d00000">[开发中]</span>

> 工作台不是「画个界面」—— 产物、作业、审批、附件在此之前全是空白。
> 本版本补齐后端地基,让 V0.7.0 真的只剩画界面与套壳。范围按 **P0 + P1**,
> 技能治理(P2)顺延。

**开工第一件事的答案(2026-08-16 查证)**

任务书要求先查 V0.2.0 的 501 占位端点是否已含 workspaces / jobs / deliverables。
**查证结果:四类端点(workspace / job / deliverable / attachment)契约里一个都没有。**

⇒ 本版本是**契约新增**,不是「把 501 换成实现」。全部走 `path.added`
(相容变更),契约冻结检查放行,**不构成破坏性变更**。
