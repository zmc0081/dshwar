...
| V0.9.5     | 移动端 SDK 客户端与可运行示例(前置:Kotlin/Swift 工具链)   | 1 周 | 待启动                                                   |
...
> 下面那句「编译不了、测不了」对 CI 不成立(ubuntu 上两条工具链都在,
> 而且 `swift-actions/setup-swift` / `setup-java` 这两个计划中的 action **不需要**)。
>
> ⇒ **缩范围这个决定本身没有被推翻,但它现在缺一个理由。**
> 移到 V0.9.5 今天是一次排期取舍,不是一次工具链约束 —— 这两者
> 在计划里长得一样,而只有前者是可以由人改主意的。**留待裁决,不自行扩范围。**
>
...
>
> ⇒ 取的办法是**缩范围**:本版本交付「SDK 模型 + 三道断言」,
> 客户端与示例移到 **V0.9.5**,前置是 Kotlin/Swift 工具链。
> Session 0 已经把那个 CI job 的规格写好了(`setup-java` + Kotlin 编译器、
> `swift-actions/setup-swift`,编译**示例工程**来验语法合法性)——
> **示例正是让第四道防线可落地的那个东西**,所以三样要一起做,不是分开补。
>
