...
> 且都是不可逆的对外动作。
>
> ✅ **首发版本号已定:`0.9.0`**(2026-08-24 裁决)。全仓已是这个号,发布时无需再改。
>
> **为什么不是 1.0.0** —— 契约本身够格,挡住它的不是契约:
>
> | 考虑                             | 判断                                                                                   |
> | -------------------------------- | -------------------------------------------------------------------------------------- |
> | `/v1` 冻结自 V0.2.0,累计破坏性 0 | ✅ 支持 —— 而且是**门禁强制**的,不是自觉                                               |
> | **项目名商标复核未完**           | ❌ **最硬的一条**:不能在一个可能改名的命名空间上承诺 API 稳定                          |
> | `console-contract` 不在冻结基线  | ❌ 它没有 `openapi.json`,冻结检查只读 `api-contract/openapi.json`;而它刚吃了破坏性变更 |
> | Kotlin / Swift 只有模型          | ❌ `sdk-kotlin@1.0.0` 会读成「稳定的 SDK」,而它是半个交付                              |
> | 治理默认装配是内存实现           | ❌ 重启即丢审计 / 用量 / 配额                                                          |
>
> **不分档**(部分包 1.0.0):判据若定为「公开面有没有强制兼容门禁」,
> 合格的只有 `api-contract` 一个 —— 27:1 换来的信号很小,
> 却拆掉了让 `check-version` 便宜的那条「统一版本号」不变式。
>
> ⚠️ 历史教训保留:本文件曾写死「V0.1.0」、后改「目标版本 0.4.5」,两次都很快过时。
> **除本段外,下文一律说「首次发布」,不要再往别处写死版本号。**
...
- [~] ~~⚠️ **补测 node-pty 在两层嵌套下是否可用**(需先装 `dsh-subprocess-local`)~~

      🚨 **已作废(2026-08-24)· 归类:前置被刻意排除。**
      它不是「忘了做」,是**依赖的东西被刻意排除了** ——
      `dsh-subprocess-local` 在 `gateway/src/runtime.ts` 的
      `DELIBERATELY_OMITTED` 里有明确理由(node-pty 原生构建;
      上游 `ProcessInspector` 的 win32 直接抛错)。
      **同一个理由已经让 `terminal-contract` job 被删过一次**(2026-08-17)。

      ⇒ 一条前置被刻意排除的待办,不会因为留在清单上就变得可做;
      它只会在每次读清单时重新消耗一次注意力。
      真要做,先推翻 `DELIBERATELY_OMITTED` 里那条理由,那才是入口
...
- [x] `git remote add origin …` 并**只推 `main`**
  ...
  >
  > 🚨 **已作废(2026-08-24)· 归类:规则被后续修法取消。**
  > 上面这条当时是对的,而后来的修法**把它取消了** —— CI 现在只调
  > `check:all` 一个入口,并且有守卫(`checkCiEnumeratesGates`)**禁止**在
  > `ci.yml` 里枚举任何门禁项。今天照这条做,会当场被那条守卫拦下。
  >
  > ⇒ **一条曾经正确、现在会把人引向违规的规则,留着比没有更糟。**
  > 今天的规矩只有一句:**门禁的唯一清单是 `check:all`,`ci.yml` 只调它。**
