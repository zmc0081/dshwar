...
网关**默认**是逻辑隔离 —— 单进程,**且自 V0.4.7 起只支持单个 principal**。
自 V0.4.5 起可以显式切到进程隔离(一 principal 一进程),**但默认不变** ——
升级不会自动改变隔离级别。**多用户部署必须显式选进程档。**
...
| 一个团队,成员互相认识   | 🚫 **拒绝启动** | 可以         |
| 免费试用 + 付费用户混跑 | 🚫 **拒绝启动** | 可以         |
| 面向公众的多租户 SaaS   | 🚫 **拒绝启动** | ⚠️ 见 §2.5   |

> ⚠️ **V0.4.7 起,逻辑档只支持单 principal。** 上面前两行曾写着「可以」——
> 那是错的,而且错得不明显:principal 到不了 agent 执行层,所有人的文件
> 都落进 `anonymous/anonymous/`,**互相静默覆盖**。这不是「互相信任就没关系」
> 的那种越界(那说的是恶意方能否跨界),这里是**根本没有分隔**。
>
> 所以配置层直接闸掉:逻辑档 + 多用户身份(多个静态令牌 / OIDC / JWT / SCIM)
> **拒绝启动**,错误信息里给出路。原因是架构限制而非待办,
> 见 [`DECISIONS/principal-scope-binding.md`](DECISIONS/principal-scope-binding.md)。
...
- **一个**(单用户部署、个人自建)→ 默认的 `logical` 就够。省下每进程 63 MB,
  不必调进程池,运维简单。
- **一个以上** → **必须开 `process`**。这不是建议,是唯一可选项:
  逻辑档配上多用户身份会拒绝启动(见 §0)。

> ⚠️ 这一节曾按「用户互相信任吗」来分。**那个判据在 V0.4.7 之后不成立** ——
> 信任与否决定的是「恶意方能不能越界」,而逻辑档下多用户的问题是
> **根本没有分隔**:所有人共用 `anonymous/anonymous/`,互相静默覆盖文件。
> 再互相信任的同事也不接受这个。
...
五次采样中位数,Linux(GitHub ubuntu-latest)与 Windows 11 各一组、11 个插件全集
(明细见 [`FEASIBILITY-REPORT-V45.md`](FEASIBILITY-REPORT-V45.md) §6):

| 指标                | Linux(生产口径) | Windows | 怎么读                                       |
| ------------------- | --------------- | ------- | -------------------------------------------- |
| 冷启动(fork → 就绪) | **~86 ms**      | ~115 ms | 每个 principal 的**第一次**请求要多等这么久  |
| 其中插件装配        | ~16 ms          | ~13 ms  | 只占五分之一 —— 优化装配代码没用             |
| 常驻内存            | **~63 MB**      | ~58 MB  | 每个**活跃 principal** 一份,不是每个会话一份 |

> ⚠️ **别记成一句「Linux 更便宜」——只有冷启动是。** fork 确实便宜 25%,
> 但**常驻内存反而高 9%**。两者由不同机制决定(进程创建与模块加载 vs
> 堆与运行时开销),没有理由同向。容量规划看的是内存那一列,所以
> **Linux 上要按更高的数算**。
>
> 这两个数由 CI 常驻门禁盯着(`scripts/measure-process-cost.mjs`),
...
```
所需内存 ≈ 网关自身(Linux 实测 80 MB) + 活跃 principal 数 × 63 MB + 操作系统
