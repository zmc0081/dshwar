...
| `team.yml`        | **进程**                              | 团队内多用户     |
| `gateway.yml`     | 插件清单,档位配在 gateway.config.json | 多用户须配进程档 |
| `enterprise.yml`  | 进程 / 容器                           | 跨信任边界       |

⚠️ **V0.4.7 起,逻辑档只支持单个 principal。** 本表上一版把 team / gateway 标成
「逻辑 · 仅限互相信任的用户」——**档位与理由都过时了**:team.yml 已改进程档,
而「互相信任」这个判据本身不成立(逻辑档下多用户是根本没有分隔,不是隔离强度不够)。

边界见 [README 的隔离模型警告](../README.md#-隔离模型警告--先读这一段)。
