...
| 📊 **性能数字取自 Linux CI 基线** | 规模对照表按 **实测 63 MB/进程**,由 CI 常驻门禁盯着(见 [决策](docs/DECISIONS/process-cost-thresholds.md))。⚠️ **这张表只算子进程** —— 网关自身(实测 78 MB)与操作系统都不在里面,按表配机器请留出余量。`maxProcesses` 自 V0.4.7 起按你的机器内存推导,不再是固定的 64 |
