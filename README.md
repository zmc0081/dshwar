...
配置与陷阱(尤其是**价格表必须配全** —— 查不到价是「算不出来」,
既不是"免费"也不再是一个 0)见 [`docs/GOVERNANCE.md`](docs/GOVERNANCE.md)。
...
涉密、内网、金融、医疗客户只有这一个选择。这不是降级方案,是差异化。

`@dshwar/llm-local` 把 Ollama / llama.cpp 的 **OpenAI 兼容端点**注册为独立
provider —— 复用上游适配器,一行模型调用代码都不写(治理层,不造引擎,
裁决记录见 [`docs/DECISIONS/llm-local-reuses-upstream-adapter.md`](docs/DECISIONS/llm-local-reuses-upstream-adapter.md))。
keyless:本地端点没有凭据可保护;**baseUrl 只接受 loopback / 私有地址**,
指向公网直接拒绝启动。

**真离线的硬边界**(不多不少,写明免得从事故里学):

| 能力                           | 离线可用?                                    |
| ------------------------------ | -------------------------------------------- |
| 浏览历史会话、产物、工作区文件 | ✅                                           |
| 本地工具执行(文件操作、脚本)   | ✅                                           |
| Agent 推理                     | ❌ **除非配置了本地模型**(此时自动降级,见下) |
| 云端 token                     | ❌ 不可用                                    |

**自动降级三态,一个都不塌缩**:云端可达 → 原样;云端不可达且本地活着 →
换本地模型(**可见**:响应头 + 审计,静默换模型是红线);两边都不可达 →
**503 明确报错**,信息里写着「装 Ollama 可离线用」。判据在**连接层**:
云端返回 500 也算可达 —— 服务器答话了,故障不在网络,降级反而掩盖真实故障。

**本地用量是统计,不是计费。** 云端 token 花的是按量收的钱,本地算力花的是
你自己的电 —— DSHWAR 没有立场替它标价。于是不存在离线额度、预授权、签名账本:
**规则不适用于不存在的东西**。账单上本地行金额恒 0 但 token 完整可见
(不计费 ≠ 隐身),统计口径与云端一致(`summarizeLocalUsage`),可做对比看板。
...
| ------ | ------------ | --------------------------------------- | ------ | ------------------ | -------------- |
| 0.9.0  | `/v1` + SCIM | 0.1.0-rc.6                              | 4.0.1  | ^22.19.0 \|\| >=24 | 端(开发中)     |
| 0.8.0  | `/v1` + SCIM | 0.1.0-rc.6                              | 4.0.1  | ^22.19.0 \|\| >=24 | 移动端 SDK     |
| 0.6.5  | `/v1` + SCIM | 0.1.0-rc.6                              | 4.0.1  | ^22.19.0 \|\| >=24 | 本地模型       |
| 0.6.0  | `/v1` + SCIM | 0.1.0-rc.6                              | 4.0.1  | ^22.19.0 \|\| >=24 | 支付           |
| 0.5.5  | `/v1` + SCIM | 0.1.0-rc.6                              | 4.0.1  | ^22.19.0 \|\| >=24 | 工作台后端     |
| 0.5.0  | `/v1` + SCIM | 0.1.0-rc.6                              | 4.0.1  | ^22.19.0 \|\| >=24 | 控制平面       |
...
|              | 范围                                                                                                   |
| ------------ | ------------------------------------------------------------------------------------------------------ |
| **MIT 开源** | 全部运行时插件、API 平面、控制平面核心、`billing-local`(只记账不收款)、`billing-stripe`(Stripe 适配器) |
| **闭源**     | **仅托管服务本体** —— `billing-hosted`(托管收款,含微信 / 支付宝等国内通道)与 DSHWAR Cloud              |

> **V0.6.0 起 Stripe 适配器开源**(此前规划为闭源)。理由:支付适配器没有
> 护城河价值,闭源它等于让自建者收不了钱 —— 直接违背下面这句承诺。
> 闭源收窄到「省掉自建」的托管服务本身。

开源用户拿到的是**可用的完整基座**(`billing-local` 出账,`billing-stripe` 收款,
webhook 验签防重放幂等齐全,自建者够用);商业客户买的是省掉自建的时间。
...
- **治理的默认装配是内存实现** —— `server.ts` 里的审计/用量/配额重启即丢。
  ...
  V0.5.0 控制平面落地
- **价格表不配全,账就算不出来** —— 查不到价的模型给 `unpriced`(不是 0):
  用量页显示 `—` 并说明有几格没配价,`billing-local` **拒绝出票**。
  本地算力不计费要显式写 `unbilled`,而不是靠「查不到价」这个副作用
