...
| 📊 **性能数字已在 Linux 复测,`maxProcesses` 默认值待重定** | 下文规模对照表按 **Linux 实测 63 MB/进程**(CI 常驻门禁,见 [决策](docs/DECISIONS/process-cost-thresholds.md))。⚠️ 但 `maxProcesses: 64` 这个默认值仍按旧的 58 MB 推出 —— 64 × 63 MB ≈ **4.0 GB** 而非 3.7 GB。**按你自己机器的内存设这个值,别用默认值** |
